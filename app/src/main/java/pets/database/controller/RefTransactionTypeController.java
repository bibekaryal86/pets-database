...
public class RefTransactionTypeController {
    ...
    public ResponseEntity<RefTransactionTypeResponse> getRefTransactionTypeById(@PathVariable("id") String id) {
        if (!hasText(id)) {
    ...
    public ResponseEntity<RefTransactionTypeResponse> saveNewRefTransactionType(@RequestBody RefTransactionTypeRequest refTransactionTypeRequest) {
        if (refTransactionTypeRequest == null) {
            return response("Error Saving Transaction Type by Invalid Request!!!");
        } else {
            return response(refTransactionTypeService.saveNewRefTransactionType(refTransactionTypeRequest));
        }
    }

    @ApiIgnore
    @PutMapping(value = "/reftransactiontype/id/{id}", produces = APPLICATION_JSON_VALUE)
    public ResponseEntity<RefTransactionTypeResponse> updateRefTransactionType(@PathVariable("id") String id,
        ...
        if (!hasText(id) || refTransactionTypeRequest == null) {
            ...
            return response(refTransactionTypeService.updateRefTransactionTypeById(id, refTransactionTypeRequest));
        }
    }

    @ApiIgnore
    @DeleteMapping(value = "/reftransactiontype/id/{id}", produces = APPLICATION_JSON_VALUE)
    public ResponseEntity<RefTransactionTypeResponse> deleteRefTransactionType(@PathVariable("id") String id) {
        if (!hasText(id)) {
    ...
    private ResponseEntity<RefTransactionTypeResponse> response(RefTransactionTypeResponse refTransactionTypeResponse) {
    ...
    private ResponseEntity<RefTransactionTypeResponse> response(String errMsg) {
        return new ResponseEntity<>(RefTransactionTypeResponse.builder()
