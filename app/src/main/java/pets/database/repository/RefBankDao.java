...
public class RefBankDao {
    ...
    public RefBank getRefBankById(String id) {
        return mongoTemplate.findOne(Query.query(Criteria.where(FIELD_NAME_ID).is(id)), RefBank.class,
    ...
    public long updateRefBankById(String id, Update update) {
        return mongoTemplate.updateFirst(Query.query(Criteria.where(FIELD_NAME_ID).is(id)), update, RefBank.class,
                COLLECTION_NAME_REF_BANK_DETAILS).getModifiedCount();
    }

    public long deleteRefBankById(String id) {
        return mongoTemplate.remove(Query.query(Criteria.where(FIELD_NAME_ID).is(id)), RefBank.class,
