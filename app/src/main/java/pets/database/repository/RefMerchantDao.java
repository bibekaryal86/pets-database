...
public class RefMerchantDao {
    ...
    public List<RefMerchant> getAllRefMerchantsByUsername(String username) {
        return mongoTemplate.find(Query.query(Criteria.where("user." + FIELD_NAME_USERNAME).is(username))
                        .with(Sort.by(Sort.Direction.ASC, FIELD_NAME_DESCRIPTION)), RefMerchant.class,
                COLLECTION_NAME_REF_MERCHANT_DETAILS);
    }

    public RefMerchant getRefMerchantById(String id) {
        return mongoTemplate.findOne(Query.query(Criteria.where(FIELD_NAME_ID).is(id)), RefMerchant.class,
                COLLECTION_NAME_REF_MERCHANT_DETAILS);
    }
    ...
    public long updateRefMerchantById(String id, Update update) {
        return mongoTemplate.updateFirst(Query.query(Criteria.where(FIELD_NAME_ID).is(id)), update, RefMerchant.class,
                COLLECTION_NAME_REF_MERCHANT_DETAILS).getModifiedCount();
    }

    public long deleteRefMerchantById(String id) {
        return mongoTemplate.remove(Query.query(Criteria.where(FIELD_NAME_ID).is(id)), RefMerchant.class,
