...
public class AccountService {
  ...
  private static final String ERROR_UPDATING_ACCOUNT =
  ...
  public AccountResponse getAllAccounts() {
    ...
    return AccountResponse.builder().accounts(allAccounts).status(status).build();
  }
  ...
  public AccountResponse getAccountsByUsername(String username) {
    ...
    return AccountResponse.builder().accounts(accounts).status(status).build();
  }
  ...
  public AccountResponse saveNewAccount(AccountRequest accountRequest) {
    ...
    try {
      newAccount =
          Account.builder()
              .refAccountType(RefAccountType.builder().id(accountRequest.getTypeId()).build())
              .refBank(RefBank.builder().id(accountRequest.getBankId()).build())
              .user(User.builder().username(accountRequest.getUsername()).build())
      ...
      if (!hasText(newAccount.getId())) {
        newAccount = null;
        status = Status.builder().errMsg("Error Saving Account, Please Try Again!!!").build();
      }
    } catch (Exception ex) {
  ...
  public AccountResponse updateAccountById(String id, AccountRequest accountRequest) {
    ...
    try {
      ...
      if (modifiedCount > 0) {
        ...
        status = Status.builder().errMsg(ERROR_UPDATING_ACCOUNT).build();
        accountResponse = AccountResponse.builder().accounts(emptyList()).status(status).build();
      }
    } catch (Exception ex) {
      logger.error("Update Account By Id: {} | {}", id, accountRequest, ex);
      status = Status.builder().errMsg(ERROR_UPDATING_ACCOUNT).message(ex.toString()).build();
      accountResponse = AccountResponse.builder().accounts(emptyList()).status(status).build();
    }
