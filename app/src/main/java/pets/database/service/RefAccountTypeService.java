...
public class RefAccountTypeService {
  ...
  public RefAccountTypeResponse getAllRefAccountTypes() {
    ...
    return RefAccountTypeResponse.builder().refAccountTypes(refAccountTypes).status(status).build();
  }
  ...
  public RefAccountTypeResponse saveNewRefAccountType(RefAccountTypeRequest refAccountTypeRequest) {
    ...
    try {
      ...
      if (!hasText(newRefAccountType.getId())) {
        ...
        status = Status.builder().errMsg("Error Saving Account Type, Please Try Again!!!").build();
      }
    } catch (Exception ex) {
  ...
  public RefAccountTypeResponse updateRefAccountTypeById(
      String id, RefAccountTypeRequest refAccountTypeRequest) {
    ...
    try {
      ...
      if (modifiedCount > 0) {
        ...
        status =
            Status.builder().errMsg("Error Updating Account Type, Please Try Again!!!").build();
        refAccountTypeResponse =
            RefAccountTypeResponse.builder().refAccountTypes(emptyList()).status(status).build();
      }
    } catch (Exception ex) {
