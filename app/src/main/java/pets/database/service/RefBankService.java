...
public class RefBankService {
  ...
  public RefBankResponse getAllRefBanks() {
    ...
    return RefBankResponse.builder().refBanks(refBanks).status(status).build();
  }
  ...
  public RefBankResponse saveNewRefBank(RefBankRequest refBankRequest) {
    ...
    try {
      ...
      if (!hasText(newRefBank.getId())) {
        newRefBank = null;
        status = Status.builder().errMsg("Error Saving Bank, Please Try Again!!!").build();
      }
    } catch (Exception ex) {
  ...
  public RefBankResponse updateRefBankById(String id, RefBankRequest refBankRequest) {
    ...
    try {
      ...
      if (modifiedCount > 0) {
        ...
        status = Status.builder().errMsg("Error Updating Bank, Please try again!!!").build();
        refBankResponse = RefBankResponse.builder().refBanks(emptyList()).status(status).build();
      }
    } catch (Exception ex) {
