...
public class RefTransactionTypeService {
  ...
  public RefTransactionTypeResponse getRefTransactionTypeById(String id) {
    ...
    return RefTransactionTypeResponse.builder()
        .refTransactionTypes(
            refTransactionType == null ? emptyList() : singletonList(refTransactionType))
        .status(status)
        .build();
  }

  public RefTransactionTypeResponse saveNewRefTransactionType(
      RefTransactionTypeRequest refTransactionTypeRequest) {
    logger.info("Before Save New Ref Transaction Type: {}", refTransactionTypeRequest);
    RefTransactionType newRefTransactionType;
    Status status = null;

    try {
      newRefTransactionType =
          RefTransactionType.builder()
      ...
      newRefTransactionType =
          refTransactionTypeDao.saveNewRefTransactionType(newRefTransactionType);

      if (!hasText(newRefTransactionType.getId())) {
        newRefTransactionType = null;
        status =
            Status.builder().errMsg("Error Saving Transaction Type, Please Try Again!!!").build();
      }
    } catch (Exception ex) {
    ...
    return RefTransactionTypeResponse.builder()
        .refTransactionTypes(
            newRefTransactionType == null ? emptyList() : singletonList(newRefTransactionType))
        .status(status)
        .build();
  }

  public RefTransactionTypeResponse updateRefTransactionTypeById(
      String id, RefTransactionTypeRequest refTransactionTypeRequest) {
    ...
    try {
      ...
      if (modifiedCount > 0) {
        ...
        status =
            Status.builder().errMsg("Error Updating Transaction Type, Please Try Again!!!").build();
        refTransactionTypeResponse =
