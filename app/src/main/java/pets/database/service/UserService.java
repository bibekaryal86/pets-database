...
public class UserService {
  ...
  public UserResponse getAllUsers() {
    ...
    return UserResponse.builder().users(allUsers).status(status).build();
  }
  ...
  public UserResponse getUserById(String id) {
    ...
    try {
      ...
      status = Status.builder().errMsg(ERROR_RETRIEVING_USER).message(ex.toString()).build();
    }
  ...
  public UserResponse updateUserById(String id, UserRequest userRequest) {
    ...
    try {
      ...
      if (modifiedCount > 0) {
        ...
        status = Status.builder().errMsg(ERROR_UPDATING_USER).build();
        userResponse = UserResponse.builder().users(emptyList()).status(status).build();
      }
    } catch (Exception ex) {
      logger.error("Update User By Id: {} | {}", id, userRequest, ex);
      status = Status.builder().errMsg(ERROR_UPDATING_USER).message(ex.toString()).build();
      userResponse = UserResponse.builder().users(emptyList()).status(status).build();
    }
