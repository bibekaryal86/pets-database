...
public class UserDao {
    ...
    public long updateUserById(String id, Update update) {
        return mongoTemplate.updateFirst(Query.query(Criteria.where(FIELD_NAME_ID).is(id)), update, User.class)
                .getModifiedCount();
    }

    public long deleteUserById(String id) {
        return mongoTemplate.remove(Query.query(Criteria.where(FIELD_NAME_ID).is(id)), User.class).getDeletedCount();
    }
}
